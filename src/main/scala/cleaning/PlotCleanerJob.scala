...
object PlotCleanerJob {
  def main(args: Array[String]): Unit = {
    ...
    val cleaned = sc.newAPIHadoopFile("s3n://sb-level-ups/spark/plot.list", classOf[TextInputFormat], classOf[LongWritable], classOf[Text], hadoopConf)
    ...
    keyedMovies.coalesce(1).saveAsTextFile("s3n://sb-level-ups/spark/plots_cleaned")
  }
}
